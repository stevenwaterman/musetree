...
export async function request(config: Config, store: NodeStore, state: NodeState) {
  if (store.type === "root") {
    return await requestInternal(config, store, [], noProcessedActiveNotes(), 0);
  }
...
async function requestInternal(config: Config, store: NodeStore, prevEncoding: MusenetEncoding, prevActiveNotesAtEnd: ProcessedActiveNotes, prevDuration: number) {
  ...
  const promises = response.map((completion: Completion) => parseCompletion(completion, truncatedEncoding, prevActiveNotesAtEnd, prevDuration));
...
async function parseCompletion(completion: Completion, prevEncoding: MusenetEncoding, prevActiveNotesAtEnd: ProcessedActiveNotes, prevDuration: number): Promise<Section> {
  ...
  instruments.forEach(instrument => Object.values(prevActiveNotesAtEnd[instrument]).forEach(note => {
    activeNotesAtStart[instrument][note.pitch] = {
      ...note,
      startTime: note.startTime - prevDuration
    }
  }));
